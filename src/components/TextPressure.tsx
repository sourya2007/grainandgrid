...
const TextPressure: React.FC<TextPressureProps> = ({
  ...
  const setSize = useCallback(() => {
    ...
    const sizeFromWidth = containerW / (chars.length / 2);
    const sizeFromHeight = containerH * 0.8;
    let newFontSize = Math.min(sizeFromWidth, sizeFromHeight);
    ...
    const animate = () => {
      ...
      if (titleRef.current) {
        ...
        spansRef.current.forEach(span => {
          ...
          // Only apply font-variation-settings if we're using variable fonts
          if ((width || weight || italic) && (wdth !== 100 || wght !== 400 || italVal !== '0')) {
